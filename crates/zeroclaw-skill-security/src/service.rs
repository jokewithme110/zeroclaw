...
use zeroclaw_log::{Action, Event, record};
            ...
            match outcome {
                UploadOutcome::Skipped { skill_id } => {
                    record!(
                        ...
                        &format!("skill scan skipped: {skill_id} (already scanned and allowed)")
                    );
                }
...
fn prepare_and_upload_skill(
    ...
    if let Some(existing) = existing.as_ref() {
        ...
        if existing.archive_sha256 != archive_sha {
            record!(
                ...
                Event::new(module_path!(), Action::Note),
                &format!(
                    "skill scan: archive changed for '{skill_id}' (old_sha={}, new_sha={})",
                    existing.archive_sha256, archive_sha
                )
            );
        }
    }
    ...
    if let Some(server_sha) = upload.file_sha256.as_deref()
        && !server_sha.eq_ignore_ascii_case(&archive_sha)
    {
        return UploadOutcome::Failed {
            skill_id,
...
fn poll_skill_once(
    ...
    if let Some(server_sha) = result.file_sha256.as_deref()
        && !server_sha.eq_ignore_ascii_case(&record.archive_sha256)
    {
...
fn list_workspace_skill_dirs(workspace_dir: &Path, install_root: &Path) -> Result<Vec<PathBuf>> {
    let mut dirs = Vec::new();

    // Scan the legacy workspace skills directory (under data_dir/skills)
    ...
    // Scan skill bundles under install_root/shared/skills/<xxx>/
    let shared_root = install_root.join("shared");
    if shared_root.exists() {
        let skills_root = shared_root.join("skills");
        if skills_root.exists() {
            for entry in std::fs::read_dir(&skills_root)
                .with_context(|| format!("failed to read {}", skills_root.display()))?
            {
                let custom_dir = entry?.path();
                if custom_dir.is_dir() {
                    // Each subdirectory under 'skills' is a custom directory (e.g., 'xxx')
                    // Skills are stored directly under this directory as <skill_name>/
                    for skill_entry in std::fs::read_dir(&custom_dir)
