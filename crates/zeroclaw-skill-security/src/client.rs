...
fn apply_runtime_proxy_to_blocking_builder(
    ...
    if let Some(url) = normalize_proxy_url(cfg.all_proxy.as_deref()) {
        if let Ok(proxy) = reqwest::Proxy::all(&url) {
            builder = builder.proxy(proxy.no_proxy(no_proxy.clone()));
        }
    }
    if let Some(url) = normalize_proxy_url(cfg.http_proxy.as_deref()) {
        if let Ok(proxy) = reqwest::Proxy::http(&url) {
            builder = builder.proxy(proxy.no_proxy(no_proxy.clone()));
        }
    }
    if let Some(url) = normalize_proxy_url(cfg.https_proxy.as_deref()) {
