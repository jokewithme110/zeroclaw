...
impl SkillScanClient {
    ...
    pub fn upload_archive(&self, archive_bytes: Vec<u8>, filename: &str) -> Result<UploadResult> {
        ...
        if !status.is_success() {
            return Err(anyhow::Error::msg(format!(
                "upload scan request failed with HTTP {status}"
            )));
        }
        ...
        let data = body
            .data
            .ok_or_else(|| anyhow::Error::msg("upload scan response missing data"))?;
        Ok(UploadResult {
    ...
    pub fn query_result(&self, task_no: &str) -> Result<ScanResult> {
        ...
        if !status.is_success() {
            return Err(anyhow::Error::msg(format!(
                "query scan result failed with HTTP {status}"
            )));
        }
        ...
        let data = body
            .data
            .ok_or_else(|| anyhow::Error::msg("scan result response missing data"))?;
        Ok(ScanResult {
            status: data.status,
            status_text: data
                .status_text
