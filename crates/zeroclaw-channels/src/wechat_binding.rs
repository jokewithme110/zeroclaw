...
fn resolve_wechat_state_dir(state_dir: Option<&str>) -> PathBuf {
    state_dir.map_or_else(
        || {
            UserDirs::new()
                .map(|user_dirs| user_dirs.home_dir().join(".zeroclaw").join("wechat"))
...
mod tests {
    use super::{
        ...
        start_wechat_qr_login, wait_for_wechat_qr_login,
    };
