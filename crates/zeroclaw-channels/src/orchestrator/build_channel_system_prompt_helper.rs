...
pub fn build_channel_system_prompt(
    ...
    inject_a2a_prompt(&mut prompt, config, &config.data_dir);
    ...
    let max_chars = 8192;
    compact_system_prompt_if_needed(&mut prompt, max_chars);

    prompt
}

fn inject_tools_prompt(prompt: &mut String, config: &Config, base_prompt: &str) {
    let agent_alias = config.resolved_runtime_agent_alias();
    let agent_config = agent_alias.and_then(|alias| config.agent(alias));
    if let Some(agent) = agent_config
        && agent.native_deferred_loading_enabled
        && let Some(start) = base_prompt.find("## Deferred Tools\n\n")
    {
...
fn inject_runtime_prompt(prompt: &mut String, _config: &Config) {
    let host =
        hostname::get().map_or_else(|_| "unknown".into(), |h| h.to_string_lossy().to_string());

    let model_name = "anthropic/claude-sonnet-4.6".to_string();
...
fn inject_bootstrap_files_prompt(prompt: &mut String, config: &Config) {
    ...
    load_openclaw_bootstrap_files(prompt, workspace_dir, bootstrap_max_chars, false);
}
