...
impl BotServiceChannel {
    ...
    fn build_ws_url(&self) -> Result<String> {
        ...
        if raw.is_empty() {
            return Err(Error::msg("bot_service ws_url is empty"));
        }
        if !raw.starts_with("ws://") && !raw.starts_with("wss://") {
            return Err(Error::msg(format!(
                "invalid bot_service ws_url scheme: {}",
                raw
            )));
        }
    ...
    fn parse_ws_host_port(ws_url: &str) -> Result<(String, u16)> {
        let without_scheme = ws_url
            ...
            .ok_or_else(|| Error::msg(format!("invalid WebSocket URL scheme: {ws_url}")))?;
        let host_port = without_scheme
            .split('/')
            .next()
            .ok_or_else(|| Error::msg(format!("missing host in WebSocket URL: {ws_url}")))?;
        ...
        let port = match parts.next() {
            Some(p) => p
                .parse::<u16>()
                .map_err(|_| Error::msg(format!("invalid port in WebSocket URL: {ws_url}")))?,
            None => {
    ...
    fn parse_proxy_host_port(proxy: &str) -> Result<(String, u16)> {
        ...
        let host = parts
            .next()
            .filter(|h| !h.is_empty())
            .ok_or_else(|| Error::msg(format!("invalid http_proxy host: {proxy}")))?
            .to_string();
        let port = match parts.next() {
            Some(p) => p
                .parse::<u16>()
                .map_err(|_| Error::msg(format!("invalid http_proxy port: {proxy}")))?,
            None => 8080,
        };
        Ok((host, port))
    }
    async fn connect(&self) -> Result<()> {
        ...
        let res = if let Some(proxy) = self.cfg.http_proxy.as_deref().map(str::trim) {
            ...
            if ws_url.starts_with("wss://") {
                zeroclaw_log::record!(
                    ERROR,
                    zeroclaw_log::Event::new("bot_service", zeroclaw_log::Action::Fail)
                        .with_category(zeroclaw_log::EventCategory::Channel)
                        .with_outcome(zeroclaw_log::EventOutcome::Failure)
                        .with_attrs(serde_json::json!({
                            "ws_url": sanitize_ws_url_for_log(&ws_url),
                            "http_proxy": proxy,
                        })),
                    "bot_service http_proxy only supports ws:// URLs",
                );
...
impl Attributable for BotServiceChannel {
    fn role(&self) -> Role {
        ...
        if chat_uuid.is_empty() {
            zeroclaw_log::record!(
                ...
                zeroclaw_log::Event::new("bot_service", zeroclaw_log::Action::Fail)
                    .with_category(zeroclaw_log::EventCategory::Channel)
                    .with_outcome(zeroclaw_log::EventOutcome::Failure),
                "Missing chatUuid in recipient",
            );
            return Err(Error::msg("bot_service: missing chatUuid in recipient"));
        }
        let message_id = self
            .last_inbound_id
            .lock()
            ...
            None => {
                zeroclaw_log::record!(
                    ...
                    zeroclaw_log::Event::new("bot_service", zeroclaw_log::Action::Fail)
                        .with_category(zeroclaw_log::EventCategory::Channel)
                        .with_outcome(zeroclaw_log::EventOutcome::Failure),
                    "Bot service websocket not connected",
                );
