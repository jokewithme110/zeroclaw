...
fn load_or_create_identity(
    ...
    let secret_store = zeroclaw_config::secrets::SecretStore::new(&zeroclaw_dir, config.secrets.encrypt);
    ...
    if path.exists() {
        ...
        id.gateway.token = id.gateway.token.as_deref().map(|v| secret_store.decrypt(v)).transpose()?;
        id.gateway.host = if id.gateway.host.is_empty() { host } else { id.gateway.host };
        ...
        id_persist.gateway.token = id.gateway.token.as_deref().map(|v| secret_store.encrypt(v)).transpose()?;
