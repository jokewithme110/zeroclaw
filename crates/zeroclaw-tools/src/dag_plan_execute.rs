...
impl DagPlanExecuteTool {
    ...
    async fn execute_task_with_snapshot(
        ...
        let tool_result = tool.execute(interpolated_args).await?;
...
mod tests {
    ...
    fn test_interpolate_dep_template() {
        ...
        results.insert(
            "cinemas",
            DagPlanTaskResult {
                id: "cinemas".to_string(),
                tool: "http_request".to_string(),
                success: true,
                output: "Cinema List: ABC, XYZ".to_string(),
                error: None,
                duration_ms: 100,
                dependencies: vec![],
            },
        ...
        let prompt = "Please format: {{dep[cinemas].output}}";
        let result = tool.interpolate_string(prompt, &refs);
        assert_eq!(result, "Please format: Cinema List: ABC, XYZ");
        ...
        let prompt2 = "Cinemas: {{dep[cinemas].output}}, Addresses: {{dep[addresses].output}}";
        let result2 = tool.interpolate_string(prompt2, &refs);
        assert_eq!(
            result2,
            "Cinemas: Cinema List: ABC, XYZ, Addresses: 123 Main St"
        );

        // Test non-existent reference (should remain unchanged)
        let prompt3 = "Data: {{dep[nonexistent].output}}";
        let result3 = tool.interpolate_string(prompt3, &refs);
        assert_eq!(result3, "Data: {{dep[nonexistent].output}}");
    }
}
