//! Connected-node registry: holds node sessions and pending request/response.
...
impl ConnectedNodeRegistry {
    ...
    fn filter_by_allowlist(&self, allowed_node_ids: &[String]) -> Vec<NodeInfo> {
        ...
        let allowed: std::collections::HashSet<_> =
            allowed_node_ids.iter().filter(|s| *s != "*").collect();
        if allowed.is_empty() {
            return list;
        }
        list.into_iter()
    ...
    async fn invoke(
        ...
        let tx = self
            ...
            .ok_or_else(|| {
                ...
                anyhow::Error::msg(format!("node '{node_id}' not connected"))
            })?;
        ...
        tx.send(msg).await.map_err(|_| {
            ...
            anyhow::Error::msg(format!("node '{node_id}' channel closed"))
        })?;
