...
impl NodesTool {
    ...
    fn resolve_node(&self, args: &Value) -> Result<String> {
        ...
        if ref_.is_empty() || ref_ == "current" || ref_ == "default" {
            return sessions.first().map(|n| n.node_id.clone()).ok_or_else(|| {
                ::zeroclaw_log::record!(
                    ERROR,
                ...
                anyhow::Error::msg("no nodes are currently connected")
            });
        }
        ...
        for n in &sessions {
            ...
            let display_name = n
                ...
                .and_then(Value::as_str)
                .filter(|s| !s.is_empty());
            if display_name.is_some_and(|dn| Self::normalize_node_key(dn) == q_norm) {
                matches.push(n);
                continue;
            }
        ...
        if matches.len() > 1 {
            ...
            ::zeroclaw_log::record!(
                ...
                ::zeroclaw_log::Event::new(module_path!(), ::zeroclaw_log::Action::Fail)
                    .with_outcome(::zeroclaw_log::EventOutcome::Failure)
                    .with_attrs(::serde_json::json!({"ref" : ref_, "matches" : names.join(", ")})),
                "ambiguous node"
            );
        ...
        ::zeroclaw_log::record!(
            ...
            ::zeroclaw_log::Event::new(module_path!(), ::zeroclaw_log::Action::Fail)
                .with_outcome(::zeroclaw_log::EventOutcome::Failure)
                .with_attrs(::serde_json::json!({"ref" : ref_})),
            "no connected node matches"
        );
    ...
    fn read_required_string(args: &Value, key: &str) -> Result<String> {
        Self::read_optional_nonempty_string(args, key)
            ...
            .ok_or_else(|| {
                ...
                anyhow::Error::msg(format!("Missing required '{key}' parameter"))
            })
    }
    ...
    fn parse_duration_expr(input: &str) -> Result<u64> {
        ...
        if raw.is_empty() {
            ...
            return Err(anyhow::Error::msg("duration must not be empty"));
        }
        ...
        let base = number.parse::<f64>().map_err(|error| {
            ::zeroclaw_log::record!(
                ERROR,
                ::zeroclaw_log::Event::new(module_path!(), ::zeroclaw_log::Action::Fail)
                    .with_outcome(::zeroclaw_log::EventOutcome::Failure)
                    .with_attrs(::serde_json::json!({"raw" : raw, "error" : error.to_string()})),
                "invalid duration"
            );
            anyhow::Error::msg(format!("invalid duration '{raw}': {error}"))
        })?;
        if !base.is_finite() || base <= 0.0 {
            ...
            return Err(anyhow::Error::msg("duration must be > 0"));
        }
    ...
    async fn build_media_save_image_params(args: &Value) -> Result<Value> {
            ...
            path_buf = std::env::current_dir()
                .map_err(|error| {
                    ::zeroclaw_log::record!(
                        ...
                        "media_saveImage: cannot resolve current dir"
                    );
                    anyhow::Error::msg(format!(
                        "media_saveImage: cannot resolve current dir: {error}"
                    ))
                })?
                .join(path_buf);
        }

        let data = tokio::fs::read(&path_buf).await.map_err(|error| {
            ::zeroclaw_log::record!(
                ERROR,
                ::zeroclaw_log::Event::new(module_path!(), ::zeroclaw_log::Action::Fail)
                    .with_outcome(::zeroclaw_log::EventOutcome::Failure)
                    .with_attrs(::serde_json::json!({"error" : error.to_string()})),
                "media_saveImage: read file"
            );
            anyhow::Error::msg(format!("media_saveImage: read file: {error}"))
        })?;
        ...
        if data.len() > MAX_FILE_SAVE_BYTES {
            ::zeroclaw_log::record!(
                ...
                ::zeroclaw_log::Event::new(module_path!(), ::zeroclaw_log::Action::Fail)
                    .with_outcome(::zeroclaw_log::EventOutcome::Failure)
                    .with_attrs(::serde_json::json!({"max_bytes" : MAX_FILE_SAVE_BYTES})),
                "media_saveImage: file too large"
            );
            return Err(anyhow::Error::msg(format!(
        ...
        let mime_type = match ext.as_str() {
            ...
            _ => {
                ::zeroclaw_log::record!(
                    ...
                    ::zeroclaw_log::Event::new(module_path!(), ::zeroclaw_log::Action::Fail)
                        .with_outcome(::zeroclaw_log::EventOutcome::Failure)
                        .with_attrs(::serde_json::json!({"format" : ext})),
                    "media_saveImage: unsupported format"
                );
                return Err(anyhow::Error::msg(format!(
        ...
        let file_name = Self::read_optional_nonempty_string(args, "filename")
            ...
            .ok_or_else(|| {
                ::zeroclaw_log::record!(
                    ...
                    ::zeroclaw_log::Event::new(module_path!(), ::zeroclaw_log::Action::Fail)
                        .with_outcome(::zeroclaw_log::EventOutcome::Failure),
                    "media_saveImage: cannot determine fileName"
                );
                anyhow::Error::msg("media_saveImage: cannot determine fileName")
            })?;
...
impl Attributable for NodesTool {
    fn role(&self) -> Role {
    ...
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        ...
        match action {
            ...
            "camera_snap" => {
                ...
                if !matches!(facing, "front" | "back" | "both") {
                    ::zeroclaw_log::record!(
                        ...
                        ::zeroclaw_log::Event::new(module_path!(), ::zeroclaw_log::Action::Fail)
                            .with_outcome(::zeroclaw_log::EventOutcome::Failure)
                            .with_attrs(::serde_json::json!({"facing" : facing})),
                        "invalid facing"
                    );
                    return Err(anyhow::Error::msg("invalid facing (front|back|both)"));
                }
                ...
                for f in &facings {
                    ...
                    let res = self
                        ...
                        .map_err(|e| {
                            ::zeroclaw_log::record!(
                                ...
                                .with_outcome(::zeroclaw_log::EventOutcome::Failure)
                                .with_attrs(
                                    ::serde_json::json!({"facing" : f, "error" : e.to_string()})
                                ),
                                "camera.snap invoke failed"
                            );
                    ...
                    let payload_obj = payload.as_object().ok_or_else(|| {
                        ::zeroclaw_log::record!(
                            ...
                            .with_outcome(::zeroclaw_log::EventOutcome::Failure),
                            "camera.snap: expected object in response"
                        );
                        anyhow::Error::msg("camera.snap: expected object in response")
                    })?;
                    let b64 = payload_obj
                        .get("base64")
                        .and_then(Value::as_str)
                        .ok_or_else(|| {
                            ::zeroclaw_log::record!(
                                ...
                                .with_outcome(::zeroclaw_log::EventOutcome::Failure),
                                "camera.snap: empty base64 in response"
                            );
                            anyhow::Error::msg("camera.snap: empty base64 in response")
                        })?;
                    ...
                    let img_bytes = base64::engine::general_purpose::STANDARD
                        ...
                        .map_err(|e| {
                            ::zeroclaw_log::record!(
                                ...
                                "camera.snap: base64 decode failed"
                            );
                            anyhow::Error::msg(format!("camera.snap: base64 decode failed: {e}"))
                        })?;
                    ...
                    if let Some(parent) = path.parent() {
                        tokio::fs::create_dir_all(parent).await.map_err(|e| {
                            ::zeroclaw_log::record!(
                                ...
                                .with_outcome(::zeroclaw_log::EventOutcome::Failure)
                                .with_attrs(::serde_json::json!({"error" : e.to_string()})),
                                "camera.snap: mkdir failed"
                            );
                            anyhow::Error::msg(format!("camera.snap: mkdir: {e}"))
                        })?;
                    }
                    tokio::fs::write(&path, &img_bytes).await.map_err(|e| {
                        ::zeroclaw_log::record!(
                            ERROR,
                            ::zeroclaw_log::Event::new(
                                module_path!(),
            ...
            "camera_clip" => {
                let node_id = self.resolve_node(&args)?;
                let facing =
                    Self::read_optional_nonempty_string(&args, "facing").unwrap_or("front");
                if !matches!(facing, "front" | "back") {
                    ...
                    return Err(anyhow::Error::msg("invalid facing (front|back)"));
                }
                    ...
                    match Self::read_optional_nonempty_string(&args, "desiredAccuracy") {
                        ...
                        Some(value) => {
                            ::zeroclaw_log::record!(
                                ...
                                .with_outcome(::zeroclaw_log::EventOutcome::Failure)
                                .with_attrs(::serde_json::json!({"value" : value})),
                                "invalid desiredAccuracy"
                            );
                            return Err(anyhow::Error::msg(
                    ...
                    Self::read_optional_nonempty_string(&args, "invokeParamsJson")
                {
                    serde_json::from_str::<Value>(raw_json).map_err(|error| {
                        ::zeroclaw_log::record!(
                            ...
                            .with_outcome(::zeroclaw_log::EventOutcome::Failure)
                            .with_attrs(::serde_json::json!({"error" : error.to_string()})),
                            "invokeParamsJson must be valid JSON"
                        );
                        anyhow::Error::msg(format!("invokeParamsJson must be valid JSON: {error}"))
                    })?
                } else {
                    serde_json::json!({})
                };
            ...
            "run" => {
                ...
                let raw_command =
                    args.get("command")
                        ...
                        .ok_or_else(|| {
                            ...
                            anyhow::Error::msg("command required (argv array)")
                        })?;
                ...
                if command.is_empty() {
                    ...
                    return Err(anyhow::Error::msg("command must not be empty"));
                }
