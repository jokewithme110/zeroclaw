...
"#,
    ...
    /// Regression test for #7025: plugin-shipped skills must be readable via
    /// read_skill using their fully-qualified `plugin:<plugin>/<skill>` name.
    ///
    /// Prior to the fix in b33f0d4a35, read_skill constructed a loader that
    /// only scanned workspace skills (and optionally open-skills/scripts),
    /// silently skipping agent-bound bundles and plugin skills. This test
    /// pins the plugin loading path: a skill-capable plugin's exposed skill
    /// name must resolve through ReadSkillTool.
    ...
    async fn reads_plugin_builtin_skill_by_qualified_name() {
        ...
        // Setup a fake skill-only plugin (no WASM required)
        let plugins_dir = tmp.path().join("plugins");
        let plugin_dir = plugins_dir.join("my-skill-plugin");
        let plugin_skill_dir = plugin_dir.join("skills/plugin-skill");
        std::fs::create_dir_all(&plugin_skill_dir).unwrap();

        // Minimal manifest declaring the "skill" capability
        std::fs::write(
            plugin_dir.join("manifest.toml"),
            r#"
name = "my-skill-plugin"
version = "1.0.0"
description = "Test plugin with builtin skills"
capabilities = ["skill"]
"#,
        )
        .unwrap();

        // Create a plugin-builtin skill with proper YAML frontmatter
        std::fs::write(
            plugin_skill_dir.join("SKILL.md"),
            "---\nname: plugin-skill\ndescription: A skill shipped inside a WASM plugin\n---\n\n# Plugin Skill\n\nThis skill is shipped inside a WASM plugin.\n",
        )
        .unwrap();

        // Enable plugins in config - use absolute path to avoid tilde expansion issues
        let plugins_dir_str = plugins_dir.to_string_lossy().to_string();
        let mut config = Config {
            plugins: PluginsConfig {
                enabled: true,
                plugins_dir: plugins_dir_str.clone(),
        ...
        // Plugin skills are namespaced as plugin:<plugin>/<skill>
        let result = tool
            .execute(json!({ "name": "plugin:my-skill-plugin/plugin-skill" }))
            .await
            .unwrap();

        assert!(
            result.success,
            "plugin-builtin skill must be readable via its qualified name; got error={:?}",
            result.error
        );
        assert!(result.output.contains("# Plugin Skill"));
        assert!(result.output.contains("shipped inside a WASM plugin"));
    }
}
