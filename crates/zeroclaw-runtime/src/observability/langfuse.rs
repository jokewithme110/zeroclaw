...
//!   - `LlmRequest`                  → caches `input_json` / `input_tools_json`
//!                                     for the next `LlmResponse`
//!   - `ToolCallStart`               → caches `arguments` for the next
//!                                     `ToolCall` on the same tool name
//!
//! I/O capture (`input_json`, `output_text`, …) is opt-in via the
//! `langfuse_include_io` config; when disabled the Generation only carries
//! model name, token usage, and timing.
...
pub struct LangfuseObserver {
    ...
    include_io: bool,
    /// Pending input JSON from the last `LlmRequest`. Consumed by `LlmResponse`.
    pending_input_json: ParkingMutex<Option<String>>,
    /// Pending input tools JSON from the last `LlmRequest`. Consumed by `LlmResponse`.
    pending_input_tools_json: ParkingMutex<Option<String>>,
}
...
impl LangfuseObserver {
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            include_io,
            pending_input_json: ParkingMutex::new(None),
            pending_input_tools_json: ParkingMutex::new(None),
        })
    }
    ...
    fn record_event(&self, event: &ObserverEvent) {
        match event {
            ...
            ObserverEvent::LlmRequest {
                ...
                // Store input for use when LlmResponse arrives.
                *self.pending_input_json.lock() = input_json.clone();
                *self.pending_input_tools_json.lock() = input_tools_json.clone();
            }
            ObserverEvent::LlmResponse {
                ...
                if self.include_io {
                    let input = self.pending_input_json.lock().take();
                    ...
                    let tools_input = self.pending_input_tools_json.lock().take();
                    if let Some(ref tools_json) = tools_input {
                        if !tools_json.is_empty() {
                            attrs.push(KeyValue::new(
                                "langfuse.observation.metadata.tool_definitions",
                                tools_json.clone(),
                            ));
                        }
                    }
                    if let Some(text) = output_text {
                        attrs.push(KeyValue::new("langfuse.observation.output", text.clone()));
                    }
                    if let Some(tc) = output_tool_calls_json {
                        if !tc.is_empty() {
                            attrs.push(KeyValue::new(
