...
cli-skills-bootstrap-templates-about = ローカルのスキルディレクトリに組み込みテンプレートスキルを初期化
...
cli-skills-bootstrap-created = {$dir} に {$count} 個のテンプレートファイルを初期化しました
cli-skills-bootstrap-existing = 組み込みテンプレートスキルはすでに {$dir} に存在します
