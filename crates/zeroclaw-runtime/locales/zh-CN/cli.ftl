...
cli-cost-about = 查看 token 与费用用量汇总
cli-cost-daily-about = 查看每日 token 用量
cli-cost-monthly-about = 查看每月 token 用量
cli-cost-daily-title = 每日 Token 用量（{$tz}）- {$date}
cli-cost-monthly-title = 每月 Token 用量（{$tz}）- {$month}
cli-cost-col-model = 模型
cli-cost-col-input = 输入
cli-cost-col-cached = 缓存
cli-cost-col-output = 输出
cli-cost-col-cost = 费用
cli-cost-total-label = 总计
cli-cost-empty = 所选时间范围内没有用量记录。
...
cli-skills-bootstrap-templates-about = 在本地技能目录中引导创建内置模板技能
cli-skills-remove-about = 移除已安装的技能
cli-skills-test-about = 为某个技能（或所有技能）运行 TEST.sh 验证
cli-skills-bootstrap-created = 已在 {$dir} 下引导创建 {$count} 个模板文件
cli-skills-bootstrap-existing = 内置模板技能已存在于 {$dir}
