...
use ra2a::types::{
    AgentCapabilities, AgentCard, AgentSkill, Message, Part, Task, TaskState, TaskStatus,
};
...
fn build_agent_skills(
    ...
    for skill in loaded {
        ...
        if filtered
            && !configured.contains(&slug)
            && !configured.contains(&name_key)
            && !configured.contains(&skill_id)
        {
            continue;
        }
        ...
        fn execute<'a>(
            ...
            Box::pin(async move {
                ...
                let input = ctx
                    ...
                    .unwrap_or_default();
                let output = zeroclaw_runtime::agent::process_message(
...
pub async fn handle_a2a_agent_card(State(state): State<AppState>) -> impl IntoResponse {
    if !state.config.read().gateway.a2a.enabled {
    ...
    if let Some(server_state) = current_a2a_server_state() {
        if let Some(method) = rpc_method_name(&body) {
            if is_streaming_method(&method) {
                if !state.config.read().gateway.a2a.stream_enabled {
